...
The source code of the service and the bot can be found [here](https://github.com/sataako).
The Telegram bot can be found on [Telegram](https://telegram.org/) by searching for @sataakobot or by going directly to the bot in web view [here](https://web.telegram.org/#/im?p=@sataakobot). Registration is needed for using Telegram. Both Telegram and the bot are free to use. The bot is not currently running on a dedicated server - don’t be surprised if its down.

## Use case
Terhi commutes by bike and is about to head home. The weather looks like it is going to rain soon and she does not want to get wet. She opens Telegram and starts a conversation with @sataakobot. She gives her current location to the bot through pressing a button in the conversation. By doing this she has subscribed to Telegram alerts from the bot in case it’s about to rain within the next hour at her location. The bot gives a warning saying it will start raining in 45 minutes. Given this information she decides she has just enough time to pedal home before the rain. When she has reached home, dry, she disables the notifications since it’s friday and she does not need the bike during the weekend.

Below you can see a demo video of the app.
<video style="width: 100%" controls=""><source src="https://github.com/sataako/sataako.github.io/blob/master/images/telegram_bot_demo.mp4?raw=true" type="video/mp4" poster="https://github.com/sataako/sataako.github.io/blob/master/images/photo6023965351362013859.jpg?raw=true"></video>
...
For visualization we also use simple vector map data by Natural Earth in 1:10 000 000 resolution. The data is published in [public domain](https://en.wikipedia.org/wiki/Public_domain_equivalent_license). We use country border and city location data as a basemap when visualizing rain rate products. The GeoTIFF format holds location data for the rain rate data which we use to map it to correct areas on the basemap.
...
## Design
The service uses the server-client model to separate data collection and analysis from the user clients. The amount of requests to the FMI API is limited, so data requesting is adjustable to fit the limits. The generation of forecasts can be a slow process, so it is done only when new data is available. The server actively polls FMI for new radar data. If new data is found, then new forecasts and images and video of the forecasts are generated based on the FMI data. The clients can poll the server for new forecast data periodically and request images of the forecasts.

<img src="https://github.com/sataako/sataako.github.io/blob/master/images/photo6023965351362013861.jpg?raw=true" width="75%" />
